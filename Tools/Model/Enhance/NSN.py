...
class Model(nn.Module):
    def __init__(self, scale_factor, in_channels, enhance_iter=4):
        super(Model, self).__init__()

        self.activation = nn.Tanh()
        self.scale_factor = scale_factor
        self.upsample = nn.UpsamplingBilinear2d(scale_factor=self.scale_factor)
        self.enhance_iter = enhance_iter
        ...
        self.mean_val = nn.Parameter(torch.zeros(1), requires_grad=False)
    ...
    def enhance(self, x, x_r):
        ...
        if self.training:
            self.mean_val.data = momentun * x_mean + (1 - momentun) * self.mean_val
        self.enhance_iter = torch.clamp(self.mean_val / x_mean + 1, min=2, max=4)
...
class L_noise(nn.Module):
    ...
    def __init__(self, k):
        ...
        self.k = k
        kernel = torch.ones((2, 1, k, k)).cuda()
        kernel[..., k // 2, k // 2] = 0
    ...
    def forward(self, enhance):
        ...
        around = F.conv2d(bool_enhance, self.weight, padding=self.k // 2, groups=2)
        # mean = self.avgpool(enhance)
        mean = self.avgpool(around)
        mask = (around < mean) & (enhance > 0)
...
class L_exp(nn.Module):
    ...
    '''
    def __init__(self,patch_size, E=.6):
    ...
    def forward(self, raw, enhance, noise_mask):
        ...
        expected_mean = torch.clamp(torch.pow(enhance_mean, 0.5), min=self.E)
        exp_loss = torch.mean(torch.pow(enhance_mean - expected_mean, 2))
        return exp_loss


class NSN_Loss(nn.Module):
    def __init__(self, patch_size=5, E=0.7, e=5, n=1):
        super(NSN_Loss, self).__init__()
        self.noise = L_noise(3) # DVSGesture 5
        self.channel = L_channel()
        self.exp = L_exp(patch_size, E)
        self.n = n
        self.e = e
    ...
    def forward(self, enhanced_image, A, raw_iamges):
        ...
        loss_exp = self.e * self.exp(raw_iamges, enhanced_image, noise_mask)
